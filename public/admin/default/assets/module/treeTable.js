...
layui.define(['laytpl', 'form', 'util'], function (exports) {
    ...
    TreeTable.prototype.initOptions = function (opt) {
        ...
        // url加载模式转为reqData模式
        if (this.options.url && this.options.reqData === undefined) {
            this.options.reqData = function (data, callback) {
                ...
                (that.options.useAdmin ? layui.admin : $).ajax({
                    ...
                    success: function (res) {
                        ...
                        if (res.code == that.options.statusCode) callback(res.data, res.totalRow);
    ...
    TreeTable.prototype.renderBodyAsync = function (d, $tr) {
        ...
        // 请求数据
        options.reqData(d, function (data, totalRow) {
            ...
            that.renderBodyData(data, d, $tr, totalRow);  // 渲染内容
        });
    };
     ...
     * @param totalRow 汇总数据
     */
    TreeTable.prototype.renderBodyData = function (data, d, $tr, totalRow) {
        var msg;
        ...
        if (d && $tr) {
            ...
            //渲染统计
            if (options.totalRow && totalRow !== undefined) {
                htmlStr += this.renderTotalRowTr(totalRow, indent);
            }
     ...
     * @returns {string}
     */
    TreeTable.prototype.renderTotalRowTr = function (d, indent) {
        var that = this;
        var options = this.options;
        if (!indent) indent = 0;
        var html = '<tr';
        var classNames = '';
        html += (' class="' + classNames + '" style="background-color:#f2f2f2;height:50px;"');
        html += (' data-index="totalRow"');
        html += (' data-indent="' + indent + '">');

        for (var i = 0; i < options.colArrays.length; i++) {
            var col = options.colArrays[i];
            var value = d[options.colArrays[i].field] ? d[options.colArrays[i].field] : '-';
            var cell = [
                '<div class="ew-tree-table-cell', col.singleLine === undefined || col.singleLine ? ' single-line' : '', '"', col.align ? ' align="' + col.align + '"' : '', '>',
                '   <div class="ew-tree-table-cell-content">', value, '</div>',
     ...
     */
    TreeTable.prototype.refresh = function (id, data, totalRow) {
        if (isClass(id) === 'Array') {
            data = id;
            id = undefined;
        }
        ...
        if (data) {  // 数据模式
            ...
            if (data.length > 0 && this.options.tree.isPidData) {  // pid形式数据
                this.renderBodyData(tt.pidToChildren(data, this.options.tree.idName, this.options.tree.pidName, this.options.tree.childName), d, $tr, totalRow);
            } else {
                this.renderBodyData(data, d, $tr, totalRow);
            }
        } else {  // 异步模式
