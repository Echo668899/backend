 ...
 */
class AudioTagService extends BaseService
{
    /**
     * 获取所有
     * @param  bool  $hot
    ...
    {
        $keyName = CacheKey::AUDIO_TAG;
        $result  = cache()->get($keyName);
        if (is_null($result)) {
            $query  = [];
            $result = AudioTagModel::find($query, [], ['sort' => -1], 0, 1000);
    ...
    public static function getGroupAttrAll()
    {
        $query  = [];
        $result = [];
        $items  = AudioTagModel::find($query, [], ['sort' => -1], 0, 1000);
    ...
    public static function getByIds(array $ids)
    {
        ...
        $rows = AudioTagModel::find(['_id' => ['$in' => $ids]], ['_id', 'name'], [], 0, 1000);
        foreach ($rows as &$row) {
            $row = [
                'id'   => strval($row['_id']),
