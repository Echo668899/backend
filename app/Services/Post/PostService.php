...
class PostService extends BaseService
{
    ...
    public static function asyncEs($id)
    {
        ...
        $row['has_images'] = !empty($row['images'])?true:false;
        $row['has_videos'] = !empty($row['videos'])?true:false;
...
//                $query['query']['bool']['must'][] = ['exists' => ['field' => 'images.url']];
                $query['query']['bool']['must'][]=['term' => ['has_images' => true]];
            } elseif ($type === 'video') {
                // 有视频
//                $query['query']['bool']['must'][] = ['exists' => ['field' => 'videos.url']];
                $query['query']['bool']['must'][]=['term' => ['has_videos' => true]];
            }
            unset($type);
        }
        if (!empty($position)) {
...
//                            'url'=>M3u8Service::encode($video['url'],'tencent')
                ...
                'has_images' => $item['has_images'] ? 'y' : 'n',
                'has_videos' => $item['has_videos'] ? 'y' : 'n',
...
//                }),
    ...
    private static function getAdItem($item)
    {
        $row = [
            ...
            'content' =>   CommonService::getCdnUrl($item['content']),
            'type' => 'ad',
            'user_id' => [],
            'tags' => [],
            'images' => [],
...
//            );
    ...
    public static function doBuy($userId,$postId)
    {
        ...
        if($money>0) {
            AccountService::reduceBalance($userRow,$orderSn,$money,8,'balance',"解锁帖子消耗:{$money},帖子Id:{$postId} ");
            if(!empty($homeRow)) {
                //创作者计算分成比例,>0才分成
    ...
    public static function handler($action, $postId, $money = null)
    {
        switch ($action) {
            case 'click':
                ...
                PostModel::updateRaw(array('$inc' => array('real_click' => 1)), array('_id' => $postId));
                ReportPostLogService::inc($postId, 'click', 1);
                break;
            case 'buy':
                PostModel::updateRaw(array('$inc' => array('buy' => 1)), array('_id' => $postId));
            ...
            case 'favorite':
                ...
                PostModel::updateRaw(array('$inc' => array('real_favorite' => 1)), array('_id' => $postId));
            ...
            case 'love':
                ...
                PostModel::updateRaw(array('$inc' => array('real_love' => 1)), array('_id' => $postId));
