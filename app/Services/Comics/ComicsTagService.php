...
class ComicsTagService extends BaseService
{
    ...
    {
        $keyName = CacheKey::COMICS_TAG;
        $result  = cache()->get($keyName);
        if (is_null($result)) {
            $query  = [];
            $result = ComicsTagModel::find($query, [], ['sort' => -1], 0, 1000);
    ...
    public static function getGroupAttrAll()
    {
        $query  = [];
        $result = [];
        $items  = ComicsTagModel::find($query, [], ['sort' => -1], 0, 1000);
    ...
    public static function getByIds(array $ids)
    {
        ...
        $rows = ComicsTagModel::find(['_id' => ['$in' => $ids]], ['_id', 'name'], [], 0, 1000);
        foreach ($rows as &$row) {
            $row = [
                'id'   => strval($row['_id']),
