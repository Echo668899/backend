...
class HanziConvert
{
    ...
    public static function convert($str, $isSc2tc = false)
    {
        ...
        array_walk($strArr, function (&$value) use ($isSc2tc) {// 循环处理字符
            if ($isSc2tc) {// 如果是简体转繁体
                $value = self::$hanziReverseDict[$value] ?? $value;
            } else {
                $value = self::$dict[$value] ?? $value;
            }
        });
     ...
     * @param bool $isSC2tc 是否是简体转繁体
     */
    private static function initConvert($isSC2tc = false)
    {
        if ($isSC2tc) {
            if (empty(self::$hanziReverseDict)) {
                self::$hanziReverseDict = array_flip(self::$dict);// 繁简体数组翻转
            }
        }
    }
}
