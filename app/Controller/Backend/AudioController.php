...
class AudioController extends BaseBackendController
{
    ...
    {
        if ($this->isPost()) {
            $result = AudioRepository::getList($_REQUEST);
    ...
    public function initData()
    {
        ...
        $this->view->setVar('statusArr', CommonValues::getAudioStatus());
        $this->view->setVar('updateStatusArr', CommonValues::getAudioUpdateStatus());
        $this->view->setVar('catArr', CommonValues::getAudioCategories());
        $this->view->setVar('tagArr', AudioTagService::getGroupAttrAll());
        $this->view->setVar('iconArr', CommonValues::getAudioIcon());
    ...
    public function detailAction()
    {
        ...
        if (!empty($id)) {
            $result = AudioRepository::getDetail($id);
            $this->view->setVar('row', $result);
            $this->view->setVar('chapterList', AudioChapterService::getChapterList($id));
        }
        $this->initData();
    }
    ...
    public function chapterDetailAction()
    {
        ...
        $chapter = AudioRepository::getChapterDetail($id);
        if (empty($chapter)) {
            $this->sendErrorResult('章节错误!');
        }
    ...
    {
        $result = AudioRepository::save($_POST);
        if ($result) {
    ...
    public function asyncAction()
    {
        ...
        $result = AudioRepository::asyncMrs($source, $idStr);
        $this->sendSuccessResult($result);
    }
    ...
    public function widgetAction()
    {
        ...
        $this->view->pick("audio/widget/{$type}");
    }
    ...
    public function doAction()
    {
        ...
        foreach ($ids as $id) {
            if (!empty($update)) {
                AudioModel::updateById($update, $id);
            }
            AudioService::asyncEs($id);
            AudioService::delCache($id);
        }
        $this->sendSuccessResult();
    }
    ...
    {
        if ($this->isPost()) {
            $result = AudioRepository::update($_POST);
            $this->sendSuccessResult($result);
        }
    ...
    {
        if ($this->isPost()) {
            $result = AudioRepository::updateOverlay($_POST);
            $this->sendSuccessResult($result);
        }
        $this->view->setVar('ids', $this->getRequest('ids', 'string'));
        $this->initData();
    ...
    {
        if ($this->isPost()) {
            $result = AudioRepository::updateRemove($_POST);
            $this->sendSuccessResult($result);
        }
        $this->view->setVar('ids', $this->getRequest('ids', 'string'));
        $this->initData();
