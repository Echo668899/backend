...
use App\Repositories\Backend\Movie\MovieKeywordsRepository;
...
class MovieKeywordsController extends BaseBackendController
{
    ...
    {
        if ($this->isPost()) {
            $result = MovieKeywordsRepository::getList($_REQUEST);
    ...
    public function detailAction()
    {
        ...
        if (!empty($id)) {
            $result = MovieKeywordsRepository::getDetail($id);
    ...
    {
        $result = MovieKeywordsRepository::save($_POST);
        if ($result) {
    ...
    public function doAction()
    {
        ...
        if ($act == 'del') {
            $ids = explode(',', $idStr);
            foreach ($ids as $id) {
                MovieKeywordsRepository::delete($id);
            }
        }
        $this->sendSuccessResult();
    }
