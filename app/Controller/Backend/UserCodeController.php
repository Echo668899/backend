...
use App\Repositories\Backend\User\UserCodeRepository;
...
class UserCodeController extends BaseBackendController
{
    ...
    {
        if ($this->isPost()) {
            $result = UserCodeRepository::getList($_REQUEST);
    ...
    {
        $this->view->setVar('userGroups', UserGroupService::getAll());
        $this->view->setVar('productGroups', UserProductService::getAll());
        $this->view->setVar('codeTypes', CommonValues::getUserCodeType());
    }
    ...
    public function saveAction()
    {
        ...
        $result = UserCodeRepository::save($_POST);
        if ($result) {
    ...
    public function doAction()
    {
        ...
        if ($act == 'del') {
            ...
            foreach ($ids as $id) {
                ...
                if ($update) {
                    UserCodeRepository::update($update);
                }
            }
        }
        $this->sendSuccessResult();
