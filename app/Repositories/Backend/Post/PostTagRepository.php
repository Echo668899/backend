...
class PostTagRepository extends BaseRepository
{
    ...
    public static function getList($request)
    {
        ...
        $fields = [];
        $count  = PostTagModel::count($query);
        $items  = PostTagModel::find($query, $fields, [$sort => $order], $skip, $pageSize);
    ...
    {
        return PostTagModel::updateById($data, $data['_id']);
    }
    ...
    {
        $row = PostTagModel::findByID(intval($id));
        if (empty($row)) {
    ...
    {
        return PostTagModel::deleteById(intval($id));
    }
}
