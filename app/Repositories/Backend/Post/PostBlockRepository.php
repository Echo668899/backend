...
class PostBlockRepository extends BaseRepository
{
    ...
    public static function getList($request)
    {
        ...
        $fields = [];
        $count  = PostBlockModel::count($query);
        $items  = PostBlockModel::find($query, $fields, [$sort => $order], $skip, $pageSize);
        foreach ($items as $index => $item) {
            ...
            $item['style']      = CommonValues::getPostBlockStyle($item['style']);
            $items[$index]      = $item;
        }
    ...
    {
        $positions = PostNavService::getAll();
    ...
    public static function save($data)
    {
        ...
        $navRow = PostNavModel::findByID($row['nav_id']);
        if (empty($navRow)) {
            throw  new BusinessException(StatusCode::PARAMETER_ERROR, '未找到相关菜单!');
        }

        if (in_array($navRow['style'], ['post_2', 'post_3'])) {
        ...
        return PostBlockModel::save($row);
    }
    ...
    {
        $row = PostBlockModel::findByID(intval($id));
        if (empty($row)) {
    ...
    {
        return PostBlockModel::deleteById(intval($id));
    }
}
