 ...
 *
 */
class PostNavRepository extends BaseRepository
{
    /**
     * 获取列表
     * @param $request
    ...
    public static function getList($request)
    {
        ...
        $fields = array();
        $count = PostNavModel::count($query);
        $items = PostNavModel::find($query, $fields, array($sort => $order), $skip, $pageSize);
        foreach ($items as $index => $item) {
            $item['is_disabled'] = CommonValues::getIs($item['is_disabled']);
            $item['position'] = CommonValues::getPostNavPosition($item['position']);
    ...
    public static function save($data)
    {
        ...
        if($row['style']=='post_2'){
            ...
            if (!json_decode($row['filter'], true)) {
                throw  new BusinessException(StatusCode::PARAMETER_ERROR, CommonValues::getPostNavStyle($row['style']).' 必须输入搜索条件!');
            }
        }
        ...
        if($row['style']=='post_3'){
            ...
            if(CommonUtil::arrayDepth(json_decode($row['filter'],true))==1){
                throw  new BusinessException(StatusCode::PARAMETER_ERROR, CommonValues::getPostNavStyle($row['style']).' 该样式务必输入数组,格式为: [{"name":"二次元","filter":{}},{"name":"二次元","filter":{}}]  其中filter为具体搜索条件,搜索条件参考模块 ');
            }

            $hasChildFilter = false;
            foreach (json_decode($row['filter'],true) as $index=>$item) {
                if(empty($item['name'])){
                    throw  new BusinessException(StatusCode::PARAMETER_ERROR, CommonValues::getPostBlockStyle($row['style'])." 特殊模块,搜索条件配置错误\n可寻求技术指导");
                }
        ...
        $result = PostNavModel::save($row);
        if($result){
            PostNavService::deleteCache();
        }
        return $result;
    }
    ...
    {
        $row = PostNavModel::findByID(intval($id));
        if (empty($row)) {
    ...
    {
        return PostNavModel::deleteById(intval($id));
    }
}
