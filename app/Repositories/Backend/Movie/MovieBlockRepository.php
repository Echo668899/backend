 ...
 */
class MovieBlockRepository extends BaseRepository
{
    /**
     * 获取列表
     * @param        $request
    ...
    public static function getList($request)
    {
        ...
        $fields = [];
        $count  = MovieBlockModel::count($query);
        $items  = MovieBlockModel::find($query, $fields, [$sort => $order], $skip, $pageSize);
        foreach ($items as $index => $item) {
            ...
            $item['style']      = CommonValues::getMovieBlockStyle($item['style']);
            $items[$index]      = $item;
        }
    ...
    {
        $positions = MovieNavService::getAll();
    ...
    public static function save($data)
    {
        ...
        $navRow = MovieNavModel::findByID($row['nav_id']);
        if (empty($navRow)) {
            throw  new BusinessException(StatusCode::PARAMETER_ERROR, '未找到相关菜单!');
        }

        if (in_array($navRow['style'], ['video_2', 'video_3'])) {
            throw  new BusinessException(StatusCode::PARAMETER_ERROR, '该菜单为列表菜单,请直接配置搜索条件,禁止添加模块!');
        }

        if ($row['style'] == 20) {
            $filter = json_decode($row['filter'], true);
            if (empty($filter['home_ids'])) {
                throw  new BusinessException(StatusCode::PARAMETER_ERROR, '样式20的搜索条件必须是home_ids!');
            }
        }
        ...
        return MovieBlockModel::save($row);
    }
    ...
    {
        $row = MovieBlockModel::findByID(intval($id));
        if (empty($row)) {
    ...
    {
        return MovieBlockModel::deleteById(intval($id));
    }
}
