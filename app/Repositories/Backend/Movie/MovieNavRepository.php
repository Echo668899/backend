 ...
 *
 */
class MovieNavRepository extends BaseRepository
{
    /**
     * 获取列表
     * @param $request
    ...
    public static function getList($request)
    {
        ...
        $fields = array();
        $count = MovieNavModel::count($query);
        $items = MovieNavModel::find($query, $fields, array($sort => $order), $skip, $pageSize);
        foreach ($items as $index => $item) {
            $item['is_disabled'] = CommonValues::getIs($item['is_disabled']);
            $item['position'] = CommonValues::getMovieNavPosition($item['position']);
            $item['created_at'] = date('Y-m-d H:i:s', $item['created_at']);
            $item['updated_at'] = date('Y-m-d H:i:s', $item['updated_at']);

            $item['style'] = value(function () use ($item) {
                $style = CommonValues::getMovieNavStyle($item['style']);
    ...
    public static function save($data)
    {
        ...
        if($row['style']=='video_2'){
            $row['filter'] = stripcslashes($row['filter']);
            if (!json_decode($row['filter'], true)) {
                throw  new BusinessException(StatusCode::PARAMETER_ERROR, CommonValues::getMovieNavStyle($row['style']).' 必须输入搜索条件!');
            }
        }

        if($row['style']=='video_3'){
            $row['filter'] = stripcslashes($row['filter']);
            if(CommonUtil::arrayDepth(json_decode($row['filter'],true))==1){
                throw  new BusinessException(StatusCode::PARAMETER_ERROR, CommonValues::getMovieNavStyle($row['style']).' 该样式务必输入数组,格式为: [{"name":"二次元","filter":{}},{"name":"二次元","filter":{}}]  其中filter为具体搜索条件,搜索条件参考模块 ');
            }

            $hasChildFilter = false;
            foreach (json_decode($row['filter'],true) as $index=>$item) {
                if(empty($item['name'])){
                    throw  new BusinessException(StatusCode::PARAMETER_ERROR, CommonValues::getMovieBlockStyle($row['style'])." 特殊模块,搜索条件配置错误\n可寻求技术指导");
                }
        ...
        $result = MovieNavModel::save($row);
        if($result){
            MovieNavService::deleteCache();
        }
        return $result;
    }
    ...
    {
        $row = MovieNavModel::findByID(intval($id));
        if (empty($row)) {
    ...
    {
        return MovieNavModel::deleteById(intval($id));
    }
}
