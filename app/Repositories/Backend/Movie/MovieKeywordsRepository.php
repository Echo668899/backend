 ...
 */
class MovieKeywordsRepository extends BaseRepository
{
    /**
     * 获取列表
     * @param        $request
    ...
    public static function getList($request)
    {
        ...
        $fields = [];
        $count  = MovieKeywordsModel::count($query);
        $items  = MovieKeywordsModel::find($query, $fields, [$sort => $order], $skip, $pageSize);
    ...
    {
        $result = MovieKeywordsModel::findByID($id);
        if (empty($result)) {
    ...
    public static function save($data)
    {
        ...
        if (MovieKeywordsModel::count(['_id' => $row['_id']]) > 0) {
            return MovieKeywordsModel::updateById($row, $row['_id']);
        }
        return MovieKeywordsModel::insert($row);
    }
    ...
    {
        return MovieKeywordsModel::deleteByID($id);
    }
}
