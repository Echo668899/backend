...
class NovelRepository extends BaseRepository
{
    ...
    public static function getList($request)
    {
        ...
        $fields = [];
        $count  = NovelModel::count($query);
        $items  = NovelModel::find($query, $fields, [$sort => $order], $skip, $pageSize);
        foreach ($items as $index => $item) {
            $item['tags'] = value(function () use ($item) {
                $tags = NovelTagService::getByIds($item['tags']);
    ...
    {
        $row = NovelChapterModel::findByID($id);
        if (empty($row)) {
            throw  new BusinessException(StatusCode::DATA_ERROR, '数据不存在!');
        }
        $mediaUrl       = ConfigService::getConfig('media_url');
        $mediaKey       = ConfigService::getConfig('media_encode_key');
        $content        = CommonUtil::httpGet($mediaUrl . $row['content']);
        $row['content'] = AesUtil::decryptRaw($content, $mediaKey);
        return $row;
    }
    ...
    public static function save($data)
    {
        ...
        $result = NovelModel::save($row);
        if ($row['_id']) {
            NovelService::asyncEs($row['_id']);
        }
        return $result;
    }
    ...
    public static function update($data)
    {
        ...
        foreach ($ids as $id) {
            if (!empty($update)) {
                NovelModel::updateById($update, $id);
                NovelService::asyncEs($id);
                NovelService::delCache($id);
            }
        }
        return true;
    }
    ...
    public static function updateOverlay($data)
    {
        ...
        foreach ($ids as $id) {
            $novelInfo  = NovelModel::findByID($id);
            $updateTags = array_merge($novelInfo['tags'] ?: [], $update['tags']);
            $updateTags = array_values(array_unique($updateTags));

            NovelModel::updateRaw(['$set' => ['tags' => $updateTags]], ['_id' => $id]);
            NovelService::asyncEs($id);
            NovelService::delCache($id);
        }
        return true;
    ...
    public static function updateRemove($data)
    {
        ...
        foreach ($ids as $id) {
            $novelInfo = NovelModel::findByID($id);
            foreach ($novelInfo['tags'] as $i => $tId) {
                if (in_array($tId, $update['tags'])) {
                    unset($novelInfo['tags'][$i]);
                }
            }
            $updateTags = array_values($novelInfo['tags']);
    ...
    public static function asyncMrs($source, $idStr)
    {
        ...
        return NovelService::asyncMrsByIds($idsArr, $source);
    }
}
