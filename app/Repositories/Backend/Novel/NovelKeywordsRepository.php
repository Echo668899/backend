...
use App\Models\Novel\NovelKeywordsModel;

/**
 * 小说关键字管理
 * @package App\Repositories\Admin
 */
class NovelKeywordsRepository extends BaseRepository
{
    /**
     * 获取列表
     * @param        $request
    ...
    public static function getList($request)
    {
        ...
        $fields = [];
        $count  = NovelKeywordsModel::count($query);
        $items  = NovelKeywordsModel::find($query, $fields, [$sort => $order], $skip, $pageSize);
    ...
    {
        $result = NovelKeywordsModel::findByID($id);
        if (empty($result)) {
    ...
    public static function save($data)
    {
        ...
        if (NovelKeywordsModel::count(['_id' => $row['_id']]) > 0) {
            return NovelKeywordsModel::updateById($row, $row['_id']);
        }
        return NovelKeywordsModel::insert($row);
    }
    ...
    {
        return NovelKeywordsModel::deleteByID($id);
    }
}
