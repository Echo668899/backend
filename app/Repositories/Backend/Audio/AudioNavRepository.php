...
class AudioNavRepository extends BaseRepository
{
    ...
    public static function getList($request)
    {
        ...
        $items = AudioNavModel::find($query, $fields, array($sort => $order), $skip, $pageSize);
    ...
    public static function save($data)
    {
        ...
        if($row['style']=='audio_2'){
            ...
            if (!json_decode($row['filter'], true)) {
                throw  new BusinessException(StatusCode::PARAMETER_ERROR, CommonValues::getAudioNavStyle($row['style']).' 必须输入搜索条件!');
            }
        }
        ...
        if($row['style']=='audio_3'){
            ...
            if(CommonUtil::arrayDepth(json_decode($row['filter'],true))==1){
                throw  new BusinessException(StatusCode::PARAMETER_ERROR, CommonValues::getAudioNavStyle($row['style']).' 该样式务必输入数组,格式为: [{"name":"二次元","filter":{}},{"name":"二次元","filter":{}}]  其中filter为具体搜索条件,搜索条件参考模块 ');
            }

            $hasChildFilter = false;
            foreach (json_decode($row['filter'],true) as $index=>$item) {
                if(empty($item['name'])){
                    throw  new BusinessException(StatusCode::PARAMETER_ERROR, CommonValues::getAudioBlockStyle($row['style'])." 特殊模块,搜索条件配置错误\n可寻求技术指导");
                }
