 ...
 */
class AudioTagRepository extends BaseRepository
{
    /**
     * 获取列表
     * @param        $request
    ...
    public static function getList($request)
    {
        ...
        $fields = [];
        $count  = AudioTagModel::count($query);
        $items  = AudioTagModel::find($query, $fields, [$sort => $order], $skip, $pageSize);
    ...
    {
        return AudioTagModel::updateById($data, $data['_id']);
    }
    ...
    {
        $row = AudioTagModel::findByID(intval($id));
        if (empty($row)) {
    ...
    {
        return AudioTagModel::deleteById(intval($id));
    }
}
