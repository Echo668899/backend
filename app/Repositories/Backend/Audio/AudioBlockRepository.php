 ...
 */
class AudioBlockRepository extends BaseRepository
{
    /**
     * 获取列表
     * @param        $request
    ...
    public static function getList($request)
    {
        ...
        $fields = [];
        $count  = AudioBlockModel::count($query);
        $items  = AudioBlockModel::find($query, $fields, [$sort => $order], $skip, $pageSize);
        foreach ($items as $index => $item) {
            ...
            $item['style']      = CommonValues::getAudioBlockStyle($item['style']);
            $items[$index]      = $item;
        }
    ...
    {
        $positions = AudioNavService::getAll();
    ...
    public static function save($data)
    {
        ...
        $navRow = AudioNavModel::findByID($row['nav_id']);
        if (empty($navRow)) {
            throw  new BusinessException(StatusCode::PARAMETER_ERROR, '未找到相关菜单!');
        }

        if (in_array($navRow['style'], ['audio_2', 'audio_3'])) {
        ...
        return AudioBlockModel::save($row);
    }
    ...
    {
        $row = AudioBlockModel::findByID(intval($id));
        if (empty($row)) {
    ...
    {
        return AudioBlockModel::deleteById(intval($id));
    }
}
