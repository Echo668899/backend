...
class AudioRepository extends BaseRepository
{
    ...
    public static function getList($request)
    {
        ...
        $fields = [];
        $count  = AudioModel::count($query);
        $items  = AudioModel::find($query, $fields, [$sort => $order], $skip, $pageSize);
        foreach ($items as $index => $item) {
            $item['tags'] = value(function () use ($item) {
                $tags = AudioTagService::getByIds($item['tags']);
            ...
            $item['status']        = CommonValues::getAudioStatus($item['status'] * 1);
            $item['update_status'] = CommonValues::getAudioUpdateStatus($item['update_status'] * 1);
    ...
    {
        $row = AudioChapterModel::findByID($id);
        if (empty($row)) {
            throw  new BusinessException(StatusCode::DATA_ERROR, '数据不存在!');
        }
        return $row;
    ...
    public static function save($data)
    {
        ...
        $result = AudioModel::save($row);
        if ($row['_id']) {
            AudioService::asyncEs($row['_id']);
        }
        return $result;
    }
    ...
    public static function updateOverlay($data)
    {
        ...
        foreach ($ids as $id) {
            $audioInfo  = AudioModel::findByID($id);
            $updateTags = array_merge($audioInfo['tags'] ?: [], $update['tags']);
            $updateTags = array_values(array_unique($updateTags));

            AudioModel::updateRaw(['$set' => ['tags' => $updateTags]], ['_id' => $id]);
            AudioService::asyncEs($id);
            AudioService::delCache($id);
        }
        return true;
    ...
    public static function updateRemove($data)
    {
        ...
        foreach ($ids as $id) {
            $audioInfo = AudioModel::findByID($id);
            foreach ($audioInfo['tags'] as $i => $tId) {
                if (in_array($tId, $update['tags'])) {
                    unset($audioInfo['tags'][$i]);
                }
            }
            $updateTags = array_values($audioInfo['tags']);
    ...
    public static function asyncMrs($source, $idStr)
    {
        ...
        return AudioService::asyncMrsByIds($idsArr, $source);
    }
}
