 ...
 *
 */
class ComicsNavRepository extends BaseRepository
{
    /**
     * 获取列表
     * @param $request
    ...
    public static function getList($request)
    {
        ...
        $fields = array();
        $count = ComicsNavModel::count($query);
        $items = ComicsNavModel::find($query, $fields, array($sort => $order), $skip, $pageSize);
        foreach ($items as $index => $item) {
            $item['is_disabled'] = CommonValues::getIs($item['is_disabled']);
            $item['position'] = CommonValues::getComicsNavPosition($item['position']);
            $item['created_at'] = date('Y-m-d H:i:s', $item['created_at']);
            $item['updated_at'] = date('Y-m-d H:i:s', $item['updated_at']);

            $item['style'] = value(function () use ($item) {
                $style = CommonValues::getComicsNavStyle($item['style']);
    ...
    public static function save($data)
    {
        ...
        $result = ComicsNavModel::save($row);
        if($result){
            ComicsNavService::deleteCache();
        }
        return $result;
    }
    ...
    {
        $row = ComicsNavModel::findByID(intval($id));
        if (empty($row)) {
    ...
    {
        return ComicsNavModel::deleteById(intval($id));
    }
}
