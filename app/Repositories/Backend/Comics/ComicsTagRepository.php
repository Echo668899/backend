...
class ComicsTagRepository extends BaseRepository
{
    ...
    public static function getList($request)
    {
        ...
        $fields = [];
        $count  = ComicsTagModel::count($query);
        $items  = ComicsTagModel::find($query, $fields, [$sort => $order], $skip, $pageSize);
    ...
    {
        return ComicsTagModel::updateById($data, $data['_id']);
    }
    ...
    {
        $row = ComicsTagModel::findByID(intval($id));
        if (empty($row)) {
    ...
    {
        return ComicsTagModel::deleteById(intval($id));
    }
}
