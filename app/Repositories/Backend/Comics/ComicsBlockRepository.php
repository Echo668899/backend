 ...
 */
class ComicsBlockRepository extends BaseRepository
{
    /**
     * 获取列表
     * @param        $request
    ...
    public static function getList($request)
    {
        ...
        $fields = [];
        $count  = ComicsBlockModel::count($query);
        $items  = ComicsBlockModel::find($query, $fields, [$sort => $order], $skip, $pageSize);
        foreach ($items as $index => $item) {
            ...
            $item['style']      = CommonValues::getComicsBlockStyle($item['style']);
            $items[$index]      = $item;
        }
    ...
    {
        $positions = ComicsNavService::getAll();
    ...
    public static function save($data)
    {
        ...
        $navRow = ComicsNavModel::findByID($row['nav_id']);
        if (empty($navRow)) {
            throw  new BusinessException(StatusCode::PARAMETER_ERROR, '未找到相关菜单!');
        }

        if (in_array($navRow['style'], ['comics_2', 'comics_3'])) {
        ...
        return ComicsBlockModel::save($row);
    }
    ...
    {
        $row = ComicsBlockModel::findByID(intval($id));
        if (empty($row)) {
    ...
    {
        return ComicsBlockModel::deleteById(intval($id));
    }
}
