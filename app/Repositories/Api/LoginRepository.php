...
class LoginRepository extends BaseRepository
{
    ...
    public static function qrcode($request)
    {
        ...
        $userRow = UserModel::findFirst(array('username' => $username));
        if (empty($userRow)) {
    ...
    private static function getLoginToken(array $userRow,$request,$action)
    {
        ...
        $update = [
            '$set' => [
                ...
                "login_device.{$deviceId}.device_id"      => $deviceId,
                "login_device.{$deviceId}.device_type"    => $deviceType,
//                "login_device.{$deviceId}.device_name"    => $deviceName,
                "login_device.{$deviceId}.device_version" => $deviceVersion,
//                "login_device.{$deviceId}.device_info"    => $deviceInfo,
                "login_device.{$deviceId}.ip"             => $loginIp,
                "login_device.{$deviceId}.login_at"       => time(),
            ],
            '$inc' => [
                'login_num' => 1,
            ]
