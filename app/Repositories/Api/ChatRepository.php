...
class ChatRepository extends BaseRepository
{
    ...
    public static function message($userId, $toId, $fromSeqid = 0, $direction = 'before', $limit = 50)
    {
        ...
        //排除我删除的消息 --前端需要使用seqid保持消息的连续性,所以已删除的消息也返回
//        $query['deleted_by'] = ['$ne' => $userId];
        ...
        foreach ($rows as &$row) {
            $row=[
                ...
                'msg_type'  =>value(function ()use($row,$userId){
                    if(in_array($userId,$row['deleted_by'])){
                        return 'delete';///特殊,消息已删除
                    }
                    if($row['is_recalled']==1){
                        return 'recall';///特殊,消息已撤回
                    }
