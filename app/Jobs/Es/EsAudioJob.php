 ...
 */
class EsAudioJob extends BaseJob
{
    ...
    public function before()
    {
        ...
        ElasticService::initIndex('audio', $analysis, $mapping);
    }
    ...
    public function run()
    {
        ...
        while (true) {
            ...
            $items = AudioModel::find($where, ['_id'], ['_id' => -1], 0, $pageSize);
            if (empty($items)) {
                break;
            }
            foreach ($items as $item) {
                if (AudioService::asyncEs($item['_id'])) {
                    LogUtil::info('Async audio to es ok:' . $item['_id']);
                } else {
                    LogUtil::error('Async audio to es error:' . $item['_id']);
                }
                $lastId = $item['_id'];
            }
        }
